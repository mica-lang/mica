...
use mica_hl::builtin_traits::iterator;
...
pub(crate) fn load_counters(engine: &mut Engine) -> Result<(), mica_hl::Error> {
   engine.add_type(
      TypeBuilder::<CountUp>::new("CountUp")
         ...
         .add_constructor("new", |ctor| {
            move |min, max| ctor.construct(CountUp::new(min, max))
         })
   ...
   engine.add_type(
      TypeBuilder::<CountDown>::new("CountDown")
         ...
         .add_constructor("new", |ctor| {
            move |max, min| ctor.construct(CountDown::new(max, min))
         })
