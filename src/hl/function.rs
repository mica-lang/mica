...
macro_rules! impl_non_varargs {
            ...
            fn into_raw_foreign_function(self) -> RawForeignFunction {
                ...
                create_rawff(move |$gc, args| {
                    ...
                    $(
                        ...
                        let $types = $arguments.get(_n).to_language_error()?;
                        #[allow(unused)]
                        let _n = _n + 1;
                    )*
                    {
    ...
    ($($types:tt),*) => {
        ...
        impl_non_varargs!(
            ...
            let this = {
                unsafe {
                    $crate::SelfFromRawValue::self_from_raw_value(arguments.raw_self()).to_language_error()?
                }
            };
        ...
        impl_non_varargs!(
            ...
            base_parameter_count 1;
            let this = {
                unsafe { S::mut_self_from_raw_value(arguments.raw_self()) }.to_language_error()?
            };
            |arguments| (this, $($types,)*);
            Result<Ret, Err>;
            |gc, result| result
